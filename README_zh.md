...
```
Phase 1: 扫描
- 扫描已配置的工作区（memory/、knowledge/、skills/）
...
Phase 3: 质量过滤
- 应用频率、收益、稳定性、边界和OpenClaw合规质量门

Phase 4: 生成报告
- 生成含Skill候选的蒸馏报告

Phase 5: 用户审核
- 你审核并确认每个候选（确认/待定/拒绝/修改）

Phase 6: 生成草稿
- 确认后，skill在staging/目录生成正式SKILL.md

Phase 7: 手动安装
- 你手动安装（每次严格确认）

Phase 8: 已安装Skill追踪
- 追踪evolution生成的skill，优化候选
```
...
- **触发清晰**：必须指定精确的触发条件
- **可执行**：必须定义具体操作
- **反馈循环**：必须定义skill如何确认执行正确

### OpenClaw合规质量门（必须通过）
- **架构契合**：符合OpenClaw最佳实践
- **工具对齐**：可使用OpenClaw的工具集执行
- **上下文适配**：范围适合作为Skill，不过宽或过窄

## 用户响应选项

对于每个候选，请回复：
- **确认 (confirm)** → 在 staging/ 目录生成正式 SKILL.md
- **待定 (defer)** → 在下次 evolution 运行时再考虑
- **拒绝 (reject)** → 记录拒绝理由（用于未来过滤）
- **修改 (modify)** → 告诉我需要修改什么
...
├── BASELINE.md          # 增量扫描基线
├── LAST_RUN.md          # 上次运行时间戳
├── evolution-installed.md # evolution生成的已安装Skill注册表
├── staging/             # 候选草稿目录
├── reports/            # 历史报告目录
